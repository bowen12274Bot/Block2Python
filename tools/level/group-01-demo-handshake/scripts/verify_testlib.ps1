...
$buildRoot = "C:/Temp/cf_group01_demo_handshake_testlib"
...
for ($i = 1; $i -le $CaseCount; $i++) {
  $total++
  $words = Get-Random -Minimum 1 -Maximum 4
  $minLen = Get-Random -Minimum 1 -Maximum 4
  $maxLen = Get-Random -Minimum 6 -Maximum 12

  $inFile = Join-Path $work ("{0:D3}.in" -f $i)
  $ansFile = Join-Path $work ("{0:D3}.ans" -f $i)
  $oufFile = Join-Path $work ("{0:D3}.ouf" -f $i)

  $cmdLine = '"' + $gen + '" --words=' + $words + ' --minLen=' + $minLen + ' --maxLen=' + $maxLen + ' ' + $i + ' > "' + $inFile + '"'
...
$badIn = Join-Path $work "bad_empty_line.in"
[System.IO.File]::WriteAllText($badIn, "", (New-Object System.Text.UTF8Encoding($false)))
...
[System.IO.File]::WriteAllText($probeWrong, "Signal: WRONG`n", (New-Object System.Text.UTF8Encoding($false)))
