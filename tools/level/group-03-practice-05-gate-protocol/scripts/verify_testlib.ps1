...
$buildRoot = "C:/Temp/cf_group03_gate_protocol_testlib_v2"
...
for ($i = 1; $i -le $CaseCount; $i++) {
  ...
  $solOutRaw = ($inputText | & $sol | Out-String)
  $bruteOutRaw = ($inputText | & $bru | Out-String)
  $solOut = $solOutRaw.TrimEnd("`r", "`n") + "`n"
  $bruteOut = $bruteOutRaw.TrimEnd("`r", "`n") + "`n"
...
$badIn = Join-Path $work "bad_invalid.in"
[System.IO.File]::WriteAllText($badIn, "abc`n", (New-Object System.Text.UTF8Encoding($false)))
...
[System.IO.File]::WriteAllText($probeWrong, "wrong`n", (New-Object System.Text.UTF8Encoding($false)))
...
if (-not $invalidInputRejected) {
  throw "validator accepted invalid input"
}
if (-not $wrongOutputRejected) {
  throw "checker accepted wrong output"
}
