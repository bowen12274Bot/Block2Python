...
from block2python.clients.bootstrap import build_configured_app, default_progress_path
...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        self._prompt.setPlaceholderText("Prompt / instructions")
        self._content_tabs.addTab(self._prompt, "Prompt")

        self._blockly = BlocklyEmbed()
        self._blockly.output_received.connect(self._on_blockly_output)
        self._content_tabs.addTab(self._blockly, "Blockly")

        self._learning = QPlainTextEdit()
        self._learning.setReadOnly(True)
        self._learning.setPlaceholderText("Learning notes")
        self._content_tabs.addTab(self._learning, "Learning")

        self._story_intro = QPlainTextEdit()
        self._story_intro.setReadOnly(True)
        self._story_intro.setPlaceholderText("Story intro")
        self._content_tabs.addTab(self._story_intro, "Story Intro")

        self._story_outro = QPlainTextEdit()
        self._story_outro.setReadOnly(True)
        self._story_outro.setPlaceholderText("Story outro")
        self._content_tabs.addTab(self._story_outro, "Story Outro")
    ...
    def _on_blockly_output(self, output: BlocklyOutput) -> None:
        ...
        self._refresh_ui_state()
        self._feedback.setPlainText("Blockly output synced to Python code.")
