...
func show_challenge(challenge_view: Dictionary) -> void:
	_base_code_editable = bool(challenge_view.get("code_editable", false))
	_base_can_open_toolbox = bool(challenge_view.get("toolbox_allowed", false))
...
func set_toolbox_lock(active: bool, status_message: String = "") -> void:
	...
	if _toolbox_locked:
		...
		status_label.text = "Challenge flow ready"

func focus_code_editor() -> void:
	if _toolbox_locked:
		return
	challenge_panel.focus_code_input()

func _apply_toolbox_lock_state() -> void:
	challenge_panel.set_code_editable(_base_code_editable and not _toolbox_locked)
