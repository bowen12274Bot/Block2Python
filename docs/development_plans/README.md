# 開發計畫（Development Plans）

> 本資料夾用來存放各主題的設計、轉型、驗證與分階段落地計畫。

## 目前文件

- `game_architecture_transition_plan.md`
- `game_data_model_plan.md`
- `game_loader_plan.md`
- `game_product_direction_plan.md`
- `game_system_architecture_plan.md`
- `game_system_architecture_implementation_plan.md`
- `game_technology_recommendation.md`
- `game_system_boundary_plan.md`
- `game_vertical_slice_plan.md`
- `technical_introduction_plan.md`
- `technical_introduction_plan_verification.md`
- `mvp_development_plan.md`
<<<<<<< HEAD
- `wasm_judge_plan.md`
=======
- `mvp_development_plan_v2.md`
- `wasm_judge_plan.md`
- `ai_tutor_skills_plan.md`
- `godot_bridge_client_guide.md`
- `godot_poc_plan.md`
- `godot_client_structure_plan.md`
- `godot_quest_map_vertical_slice_plan.md`
- `godot_screen_separation_plan.md`
>>>>>>> main

## 命名規則與延伸

- 主計畫：`<topic>_plan.md`
- 驗證文件：`<topic>_plan_verification.md`
- 評估或 review 文件：`<topic>_plan_review.md`
- 若需保留日期版本：`<topic>_plan_YYYY_MM_DD.md`

若要撰寫新的開發計畫，請先參考 `docs/contributing/ai_collaboration.md` 與相關 skill 指引，避免與既有計畫衝突。
